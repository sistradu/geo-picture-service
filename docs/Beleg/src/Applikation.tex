...
\\
Ein weiterer interessanter Punkt hängt mit der Anzeige der hochgeladenen Bilder im DataGrid von Primefacews zusammen. In diesem DataGrid wird durch eine Liste von Bilder iteriert. (Vergleiche dazu Listing \autoref{lstDataGrid} auf Seite \pageref{lstDataGrid}.) Man kann darüber dann auf die einzelnen Informationen des Bildes zugreifen. Leider funktioniert die nicht mit den Bilddaten, die als StreamedContent vorliegen. Wo die Ursache für dieses Verhalten liegt konnte leider auch durch eine Internetrecherche nicht geklärt werden. Allerdings wurde im Form von Primefaces ein Workaround angegeben. Es muss nämlich im Controller eine Methode bereitgestellt werden, die den StreamedContent lädt. Diese Methodewird für jedes Bild aufgerufen. Dazu muss dieser Methode mittels dem Tag "`f:param"' in der Webseite der Bildname beim Aufruf übergeben werden. Den Code der HTML-Seite sieht man in Listing \autoref{lstFParam}. Der Code der Methode ist im Listing \autoref{lstGetImage} zu sehen. Beide Listings befinden sich im Anhang ab Seite \pageref{lstFParam}. Wie in dem Listing der Methode zu sehen ist muss immer noch ein Standardbild angegeben werden, falls ein Bild nicht von der Festplatte geladen werden kann. Wird dieses Standardbild nicht angegeben zeigt die DataGrid-Komponente kein Bild an, obwohl der Pfad richtig übergeben wird. Der Grund dafür liegt darin, dass Primefaces pro Bild drei Requests macht. Bei den ersten beiden Request ist der Parameter "`null"', erst beim dritten kommt der Parameter mit. Also wird bei den ersten Requests das Standardbild geholt und erst beim letzten Request das richtige Bild. Sobald kein Standardbild angegeben ist, wird wahrscheinlich in Primefaces eine NullPointerException geworfen, die das Anzeigen des eigentlichen Bildes verhindert.
